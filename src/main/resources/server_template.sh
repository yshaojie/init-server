...
start_server() {
    ...
    #start the server
    java ${server_jvm_args} -Ddubbo.registry.file=${server_home}.dubbo.properties -Ddisconf.env=${disconf_env} -Ddisconf.user_define_download_dir=${disconf_user_define_download_dir} -Dserver_ip=${server_ip} -Dstart_time="${start_time}" -Dserver_home=${server_home} -Dserver_log_home=${server_log_home} -Dserver_resources=${server_resources} -Dserver_name=${server_name} ${system_props} ${server_main_class} ${server_args} >> ${server_log_home}/${server_name}.log 2>&1 &
    sleep 3
    get_server_pids
    if [ ${#pids[*]} -gt 1 ]; then
